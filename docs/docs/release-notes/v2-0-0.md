...
+ New class `pyromod.PyromodConfig`. It′s a class with some static properties that hold some tweaks and error handlers.
  - `timeout_handler`: a callback that gets executed instead of raising _asyncio.TimeoutError_ (now `pyromod.ListenerTimeout`). It receives `(identifier, listener_data, timeout)` as arguments, and `ask()` timeouts also expose the sent prompt message through `listener_data.sent_message` and the optional `sent_message` argument when the handler signature supports it.
