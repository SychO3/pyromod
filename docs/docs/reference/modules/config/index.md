...
|------------------------------|----------|-------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `timeout_handler`            | callable | None                                                  | An optional callback function that can be set to handle timeouts for listeners. It receives `identifier`, `listener`, and `timeout`, and can also receive `sent_message` for `ask()` timeouts when supported by the handler signature. |
