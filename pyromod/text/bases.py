...
def escape_html(text: str) -> str:
    """Escape HTML-sensitive characters while preserving existing entities."""
    ...
    for match in HTML_ENTITY_RE.finditer(text):
        parts.append(escape_fragment(text[last:match.start()]))
        parts.append(match.group(0))
        last = match.end()
    parts.append(escape_fragment(text[last:]))
    return "".join(parts)
